...
// {5F4CE6C1-DD52-43CC-A880-87B69460A360}
EFI_GUID mPeiTraceHubHandleGuid = {0x5f4ce6c1, 0xdd52, 0x43cc, {0xa8, 0x80, 0x87, 0xb6, 0x94, 0x60, 0xa3, 0x60}};
  ...
  )
{
  ...
  sven_handle_t             TraceHubHandleData;
  EFI_HOB_GUID_TYPE         *GuidHob;

  TraceHubHandle = NULL;
  GuidHob        = GetFirstGuidHob (&mPeiTraceHubHandleGuid);
  if (GuidHob != NULL) {
    //
    ...
    SVEN_INIT_HANDLE_STATE (&TraceHubHandleHeader, &TraceHubHandleData, &TraceHubPlatformData);
    if (TraceHubHandleData.svh_platform.TraceHubPlatformData.MmioAddr == NULL) {
      return NULL;
    }

    TraceHubHandle = (psven_handle_t) BuildGuidHob (&mPeiTraceHubHandleGuid, sizeof (sven_header_t)+sizeof (sven_handle_t));
    CopyMem (TraceHubHandle, &TraceHubHandleData, sizeof (sven_handle_t));
    TraceHubHandle->svh_header = (psven_header_t)((UINT8 *)TraceHubHandle + sizeof (sven_handle_t));
